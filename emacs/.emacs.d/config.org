...
Setting up ~package.el~ is straightforward: given that ~package-enable-at-startup~ was set to ~t~ in the /early init file/, we do not need to require it or initialize it here.
Instead, we add /MELPA/ as an additional package archive to ELPA.
