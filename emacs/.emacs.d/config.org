...
#+begin_src emacs-lisp :tangle "init.el"
  ...
  (setq default-frame-alist
      (if (eq system-type 'darwin)
          '((ns-transparent-titlebar . t)      ; Make titlebar blend with buffer
            (ns-appearance . dark)              ; Use dark titlebar (or 'light)
...
I use [[https:www.orgroam.com/][Org Roam]] for my note-taking needs.

The idea is to create several nodes with links between them. I will call these connections edges and links interchangeably.

To Org Roam, a node is defined as

#+begin_quote
    A node is any headline or top level file with an ID.
#+end_quote

We setup Org Roam to use the following commands:
- ~C-c n f~ : Find or create a note
- ~C-c n i~ : Insert a link to another note
- ~C-c n l~ : Toggle the backlinks buffer (shows connections)
- ~C-c n c~ : Capture a new note using templates
- ~C-c n j~ : Jump to today's daily note
- ~C-c n g~ : View the graph of your notes
...
Organizing my bibliography and references through Emacs, more specifically =org-ref=, appeals to me.
...
#+begin_src emacs-lisp :tangle "modules/slp-mail.el" :mkdirp yes
  (load "~/.emacs.d/private.el" t)

  (use-package mu4e
    :config
    (setq user-full-name "Sebastian Larsen Prehn"
          user-mail-address "slp@sebastianprehn.dk"
          mu4e-user-mail-address-list (list (or my-email-address "user@example.org")
                                        "slp@sebastianprehn.dk")
          ...
          smtpmail-smtp-user (or my-email-address "user@example.org")
...
#+begin_src emacs-lisp :tangle "modules/slp-mail.el"
  ;; Org-mode integration
